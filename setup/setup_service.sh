...
/opt/homebrew/bin/sshpass -p $PASS ssh -S -p $TARGET_SSH_PORT $TARGET_IP 'sudo mv /home/ggomes/start_tastic.sh /usr/local/bin/start_tastic.sh && sudo chmod +x /usr/local/bin/start_tastic.sh'
/opt/homebrew/bin/sshpass -p $PASS scp -P $TARGET_SSH_PORT tastic.service $TARGET_IP:/home/ggomes/tastic.service
/opt/homebrew/bin/sshpass -p $PASS ssh -S -p $TARGET_SSH_PORT $TARGET_IP 'sudo mv /home/ggomes/tastic.service /etc/systemd/system/tastic.service'
...
/opt/homebrew/bin/sshpass -p $PASS ssh -S -p $TARGET_SSH_PORT $TARGET_IP 'sudo systemctl enable tastic'
...
/opt/homebrew/bin/sshpass -p $PASS ssh -S -p $TARGET_SSH_PORT $TARGET_IP 'tail -n50 /home/ggomes/start_tastic.log'
