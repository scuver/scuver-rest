#!/bin/bash -ex
TARGET_SSH_PORT=${1:-22}
TARGET_IP=${2:-192.168.1.199}
...
/opt/homebrew/bin/sshpass -p $PASS scp -P $TARGET_SSH_PORT start_tastic_linux.sh $TARGET_IP:/home/ggomes/start_tastic_linux.sh
/opt/homebrew/bin/sshpass -p $PASS ssh -p $TARGET_SSH_PORT $TARGET_IP 'sudo mv /home/ggomes/start_tastic_linux.sh /usr/local/bin/start_tastic_linux.sh && sudo chmod +x /usr/local/bin/start_tastic_linux.sh'
...
/opt/homebrew/bin/sshpass -p $PASS scp -P $TARGET_SSH_PORT tastic.service $TARGET_IP:/home/ggomes/tastic-linux.service
/opt/homebrew/bin/sshpass -p $PASS ssh -p $TARGET_SSH_PORT $TARGET_IP 'sudo mv /home/ggomes/tastic-linux.service /etc/systemd/system/tastic-linux.service'
...
/opt/homebrew/bin/sshpass -p $PASS ssh -p $TARGET_SSH_PORT $TARGET_IP 'sudo systemctl enable tastic-linux'
