...
PINGGY_COMMAND="$AUTOSSH_BIN -M 0 -N -p 443 -R0:localhost:3222 -L4300:localhost:4300 -o StrictHostKeyChecking=no -o ServerAliveInterval=30 B1Kely8PXDF@a.pinggy.io"
RETRY_COUNT=0
MAX_RETRIES=5
...
sleep 1
until $PINGGY_COMMAND >> $LOGFILE 2>&1 || [ $RETRY_COUNT -eq $MAX_RETRIES ]; do
    if grep -q "Network is unreachable" $LOGFILE; then
        echo "Retry PINGGY_COMMAND tunnel due to network issue ($RETRY_COUNT/$MAX_RETRIES)" >> $LOGFILE
        RETRY_COUNT=$((RETRY_COUNT + 1))
        sleep 10
    else
        echo "PINGGY_COMMAND authentication failed" >> $LOGFILE
    fi
done
